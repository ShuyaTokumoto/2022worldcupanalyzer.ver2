"""
Football Analytics Dashboard v3
================================
v2からの主な改善:
1. ① 攻撃プロセス: CB過大評価を修正
   - パス送り手のxT利得を削減（*3 → *2）
   - 受け手のxT利得を強化（*5 → *8）
   - キャリー（ドリブル前進）の係数を最大に（*15）
   - フリック・ターン等のショートアクションにボーナス
2. ② 守備プロセス: CBポジション補正
   - 自陣のデュエル/クリアランスの係数を大幅削減（CB有利を是正）
   - ハイプレス（相手陣でのアクション）を大きく優遇
3. 標準化: Zスコア廃止 → 全試合プールで絶対評価
   - 試合起動時にバックグラウンドで全64試合を事前分析
   - 選手スコアを全WC選手の分布で標準化 → 試合比較・善戦評価が可能
4. UI: 全WC選手ランキングページを追加
"""
...
# =========================================================
# コア分析関数（生スコアを返す・標準化なし）
# =========================================================
def compute_raw_scores(match_id: int) -> pd.DataFrame | None:
    """
    1試合を分析し、選手ごとの生スコアDataFrameを返す。
    標準化は全試合プールで後処理するため、ここでは行わない。
    """
    ...
    pa  = dict.fromkeys(scores.index, 0.0)  # ① 攻撃プロセス
    pd_ = dict.fromkeys(scores.index, 0.0)  # ② 守備プロセス
    ca  = dict.fromkeys(scores.index, 0.0)  # ③ 得点近接
    cd  = dict.fromkeys(scores.index, 0.0)  # ④ 失点近接
    ...
    # ① 攻撃プロセス
    # 設計思想:
    #   - xT利得は「受け手」を重く評価 → 前線選手が自然に高くなる
    #   - 送り手はロング・スルー等の質の高いパスのみ評価
    #   - キャリー（ドリブル前進）を最優先係数に
    #   - 自陣ロストは送り手ペナルティ（CBのリスクを反映）
    # =========================================================
    for _, ev in passes.iterrows():
        ...
        if pd.isna(ev.get("pass_outcome")):  # 成功パス
            if xt_gain > 0:
                # 送り手: xt利得の小さい係数（CBのルーティンパスを抑制）
                pa[passer] += xt_gain * 2.0
                # 受け手: xt利得の大きい係数（前線に届いた価値を受け手に）
                if receiver in pa:
                    pa[receiver] += xt_gain * 8.0

            # ファイナルサード侵入（質の高いパスへのボーナス）
            if sx < 80 <= ex:
                pa[passer] += 1.0
                if receiver in pa:
                    pa[receiver] += 0.5
    ...
    # キャリー（ドリブル前進）: 係数を最大に → ドリブラーの評価を最優先
    ...
    for _, ev in carries.iterrows():
        ...
        xt_gain = get_xt(ex, ey) - get_xt(sx, sy)
        if xt_gain > 0:
            pa[ev["player"]] += xt_gain * 15.0  # 最大係数
    ...
    # ② 守備プロセス
    # 設計思想:
    #   - 高い位置でのアクション（ハイプレス）を最大評価
    #   - 自陣のデュエル/クリアランスは低係数（CBが自動的に稼ぐのを抑制）
    #   - インターセプト（読みの守備）は位置を問わず高評価
    # =========================================================
    def def_zone_weight(location, action_type="default"):
        """
        action_type別・エリア別の守備重み
        - interception: 読みの守備は場所問わず高い
        - duel/clearance: 自陣は大幅削減（CBルーティン抑制）
        - recovery: 中程度
        """
        try:
            x = float(location[0])
        ...
        if action_type == "interception":
            # インターセプト: ハイプレス最高、自陣も一定評価
            if x >= 80:   return 3.0   # アタッキングサード（ハイプレス）
            elif x >= 60: return 2.0   # ミドルサード前方
            elif x >= 40: return 1.5   # ミドルサード後方
            else:         return 1.2   # 自陣

        elif action_type == "duel":
            # デュエル: 自陣は大幅削減（CBのルーティンデュエルを抑制）
            if x >= 80:   return 2.5   # ハイプレス
            elif x >= 60: return 1.5
            elif x >= 40: return 0.8
            else:         return 0.3   # 自陣は低い（CBルーティン抑制）

        elif action_type == "clearance":
            # クリアランス: 非常ベースの守備のみ評価、自陣ルーティンは最低
            if x < 20:    return 2.0   # ゴールライン際の危機回避
            elif x < 40:  return 0.5   # 自陣ルーティン（低い）
            else:         return 0.8
    ...
    for _, ev in pressures.iterrows():
        if ev["player"] in pd_:
            try:
                x = ev["location"][0]
                if x >= 80:
                    pd_[ev["player"]] += 0.5  # ハイプレスのみ評価
            except (TypeError, IndexError):
                pass

    # =========================================================
    # ③ 得点近接 (Critical Attack)
    # =========================================================
    ...
    for _, s in shots.iterrows():
        if s.get("shot_outcome") == "Goal":
            t = s["team"]
            team_goals[t] = team_goals.get(t, 0) + 1
    ...
    for t in events["team"].dropna().unique():
        opp_teams = [x for x in events["team"].dropna().unique() if x != t]
        opp = opp_teams[0] if opp_teams else t
...
def build_pool():
    """
    全WC試合を分析し、全選手の生スコアプールを構築。
    このプールの平均・標準偏差で標準化 → 絶対評価が可能に。
    """
    matches = sb.matches(competition_id=43, season_id=106)
    all_scores = []
    match_meta = {}  # match_id → (home, away, home_score, away_score)
...
# =========================================================
st.set_page_config(
    page_title="Football Analytics v3",
    layout="wide",
    page_icon="⚽"
)
...
st.markdown(
    "**①攻撃プロセス ②守備プロセス ③得点近接 ④失点近接 ⑤Luck**  \n"
    "2022 FIFA ワールドカップ 全64試合 ｜ 全選手絶対評価版"
)
...
all_match_ids = list(match_meta.keys())
labels = []
label_to_mid = {}
...
selected_label = st.sidebar.selectbox("試合を選択", labels)
...
if page == "🔍 試合分析":
    ...
    def render_team_card(col, team, data):
        with col:
            ...
            st.markdown(
                f"🎯 **得点Luck:** `{lk_a:+.2f}` "
                + ("✨確率超えゴール" if lk_a > 0.3 else "⚡標準的" if lk_a > -0.3 else "😬決定機逸し")
            )
            st.markdown(
                f"🛡️ **守備Luck:** `{lk_d:+.2f}` "
                + ("🧱神守備" if lk_d > 0.3 else "⚡標準的" if lk_d > -0.3 else "💥守備崩壊")
            )

            proc = data.get("プロセス合計(絶対)", 0)
            crit = data["クリティカル合計"]
            st.info(
                f"ゲーム支配力(プロセス絶対値): **{proc:+.2f}**  \n"
                f"決定局面支配(クリティカル): **{crit:+.2f}**"
            )
    ...
    # プロセス比較
    proc_cats = ["攻撃プロセス合計", "守備プロセス合計", "プロセス合計(絶対)"]
    x = np.arange(len(proc_cats))
    w = 0.35
    h_vals = [team_sum[home_t].get(c, 0) for c in proc_cats]
    a_vals = [team_sum[away_t].get(c, 0) for c in proc_cats]
    ...
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(proc_cats, rotation=10, ha="right", fontsize=9)
    axes[0].legend()
    axes[0].set_title("プロセス指標 比較（絶対評価）")

    crit_cats = ["攻撃クリティカル", "守備クリティカル", "クリティカル合計",
                 "⑤得点Luck(決定力)", "⑤守備Luck(死守度)"]
    x2 = np.arange(len(crit_cats))
    h_vals2 = [team_sum[home_t].get(c, 0) for c in crit_cats]
    a_vals2 = [team_sum[away_t].get(c, 0) for c in crit_cats]
    ...
    axes[1].set_xticklabels(crit_cats, rotation=10, ha="right", fontsize=9)
    axes[1].legend()
    axes[1].set_title("クリティカル ＆ Luck 比較")
    ...
    all_cols = ["player", "総合プロセス(①+②)", "①攻撃プロセス", "②守備プロセス",
                "総合クリティカル(③+④)", "③得点近接", "④失点近接"]

    with tab1:
        st.caption("プロセス値は全WC選手の平均=0、±1が標準偏差。絶対評価のため試合をまたいだ比較が可能です。")
        st.dataframe(fmt_df(df_team[all_cols], "総合プロセス(①+②)"), use_container_width=True)

    with tab2:
        st.caption("攻撃プロセス：xT受益（受け手重み）＋ドリブル前進。CBは低くなるよう設計。")
        st.dataframe(fmt_df(df_team[["player", "①攻撃プロセス"]], "①攻撃プロセス"), use_container_width=True)

    with tab3:
        st.caption("守備プロセス：ハイプレスを最高評価。自陣デュエル・クリアランスは低係数。")
        st.dataframe(fmt_df(df_team[["player", "②守備プロセス"]], "②守備プロセス"), use_container_width=True)

    with tab4:
        st.caption("クリティカル：③得点近接は正値が高い。④失点近接は正値がセーブ貢献、負値がミス。")
        df_c = df_team[["player", "③得点近接", "④失点近接", "総合クリティカル(③+④)"]].sort_values("総合クリティカル(③+④)", ascending=False)
        st.dataframe(
            df_c.style
    ...
    if sel_p:
        ...
        for player, color in zip(sel_p, colors):
            if player not in df_norm.index:
                continue
            vals = df_norm.loc[player, radar_dims].tolist() + [df_norm.loc[player, radar_dims[0]]]
            ax_r.plot(angles, vals, "o-", lw=2, label=player, color=color)
            ax_r.fill(angles, vals, alpha=0.15, color=color)

        ax_r.set_xticks(angles[:-1])
        ax_r.set_xticklabels(radar_dims, size=8)
        ax_r.set_ylim(0, 1)
        ...
        ax_r.set_title(f"{target_team} 選手レーダー（全WCパーセンタイル）", size=11, pad=15)
        plt.tight_layout()
        st.pyplot(fig_r)
...
# =========================================================
    ...
    st.info(
        "全64試合・全選手のスコアを同一基準で標準化。"
        "プロセス値は全WC平均=0、単位は標準偏差（SD）。"
        "同一試合内の相対評価だった旧版と異なり、**試合を超えた絶対比較**が可能です。"
    )

    rank_tab1, rank_tab2, rank_tab3, rank_tab4 = st.tabs([
        "🥇 総合プロセス", "⚡ 攻撃プロセス", "🛡️ 守備プロセス", "🎯 クリティカル"
    ])

    # フィルター
    team_filter = st.sidebar.text_input("チーム名フィルター（全ランキング共通）", "")

    def get_ranked(col, top_n=50):
        df = pool_df[["player", "team", "match_id", col]].copy()
        if team_filter:
            df = df[df["team"].str.lower().str.contains(team_filter.lower())]
        # 選手ごとに試合をまたいで合算（複数試合出場を反映）
    ...
    with rank_tab1:
        st.caption("複数試合を合算したトータルプロセス貢献ランキング")
        df_rank = get_ranked("総合プロセス(①+②)")
        st.dataframe(
            df_rank.style.background_gradient(subset=["総合プロセス(①+②)"], cmap="RdYlGn")
            .format({"総合プロセス(①+②)": "{:+.3f}"}),
            use_container_width=True
        )

    with rank_tab2:
        st.caption("前進・ドリブル・受け手貢献ランキング（CBは自然に低スコア）")
        df_rank2 = get_ranked("①攻撃プロセス")
        st.dataframe(
            df_rank2.style.background_gradient(subset=["①攻撃プロセス"], cmap="YlGn")
            .format({"①攻撃プロセス": "{:+.3f}"}),
            use_container_width=True
        )

    with rank_tab3:
        st.caption("ハイプレス・インターセプト重視ランキング（自陣ルーティンは低係数）")
        df_rank3 = get_ranked("②守備プロセス")
        st.dataframe(
            df_rank3.style.background_gradient(subset=["②守備プロセス"], cmap="Blues")
            .format({"②守備プロセス": "{:+.3f}"}),
            use_container_width=True
        )

    with rank_tab4:
        st.caption("得点・失点局面への直接関与度（③＋④合算）")
        df_rank4 = get_ranked("総合クリティカル(③+④)")
        st.dataframe(
            df_rank4.style.background_gradient(subset=["総合クリティカル(③+④)"], cmap="Purples")
            .format({"総合クリティカル(③+④)": "{:+.3f}"}),
            use_container_width=True
        )
...
# 指標説明
# =========================================================
with st.expander("📖 指標設計と算出方法（v3改善点）"):
    st.markdown("""
    ### v3の主な改善点

    | 改善項目 | v2 | v3 |
    |----------|----|----|
    | ①攻撃プロセス 送り手係数 | xT × 10 | xT × 2（抑制） |
    | ①攻撃プロセス 受け手係数 | xT × 5 | xT × 8（強化） |
    | ①攻撃プロセス キャリー係数 | xT × 12 | xT × 15（最大） |
    | ①攻撃プロセス ドリブル成功 | なし | ＋1.5pt（前線ボーナス） |
    | ②守備プロセス 自陣デュエル重み | 0.3〜0.8 | 0.3（CB抑制） |
    | ②守備プロセス ハイプレス | 2.0 | 2.5（強化） |
    | ②守備プロセス プレッシャー | なし | ＋0.5pt（80m以上のみ） |
    | 標準化 | 試合内Zスコア（相対） | **全64試合プールで絶対評価** |
    
    ### 指標の意味

    | 指標 | 設計思想 |
    |------|---------|
    | ① 攻撃プロセス | 「受け手」と「ドリブラー」が高くなるよう設計。CBのルーティンパスは低係数 |
    | ② 守備プロセス | ハイプレス・インターセプトを最評価。自陣クリアランスは低係数 |
    | ③ 得点近接 | xGチェーン（シューター50%・直前30%・前々20%）+ ヘディング補正 |
    | ④ 失点近接 | セーブ/ブロック（xG加重）マイナス ターンオーバー被xGペナルティ |
    | ⑤ Luck | 得点−xG（決定力） / 被xG−失点（守備の幸運）。純粋な確率乖離度 |

    ### 絶対評価の意味
    - プロセス値 **+1.0** = 全WC選手の上位約16%
    - プロセス値 **0.0** = WC平均的な選手
    - プロセス値 **−1.0** = 平均を大きく下回る
    - 「負けたが善戦した」= プロセス合計が高いのに運(Luck)が負けに作用したケース
    """)
