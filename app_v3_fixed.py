...
SB_LOGO_DARK_URL = (
    "https://raw.githubusercontent.com/statsbomb/logos/main/"
    "HudlStatsbomb_Python_Dark.svg"
)

def render_sb_sidebar():
    """サイドバー下部に StatsBomb クレジットを表示"""
    st.sidebar.markdown("---")
...
def render_sb_footer():
    ...
    with col_text:
        st.markdown(
            "**データソース: StatsBomb Open Data**  \n"
            "本アプリは StatsBomb Services Ltd. が無償公開している "
            "[StatsBomb Open Data](https://github.com/statsbomb/open-data) を使用しています。  \n"
            "[StatsBomb Open Data User Agreement](https://github.com/statsbomb/open-data/blob/master/LICENSE.pdf) "
            "に基づき、非商業目的に限り利用可能です。  \n"
